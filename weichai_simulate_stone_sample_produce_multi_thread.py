...
def mergeStone2Img(stone_img, stone_mask, tar_img, tar_cor):
    ...
    print('111111111111111%s' % tar_cor)
...
def produce_data(pre_root_dir, left_or_right, model_list, stone_img_dir):
    ...
    positive_label_dir = os.path.join(aim_positive_dir, 'labels')
    ...
    for aim_model in model_list:
        ...
        positive_label_txt = os.path.join(positive_label_dir, '%s_positive_label.txt' % aim_model)
        ...
        for sub_model in sub_models_list:
            ...
            error_num = len(error_img_path_list)
            ...
            for error_index in range(error_num):
                ...
                # error_img = cv2.imread(src_img_path, 0)
                ...
                fake_stone_rect_list = [[int(stone_x_center), int(stone_y_center), stone_width, stone_height]]
                print(fake_stone_rect_list)


                print('开始生成数据...')
                rect_index = 0
                for stone_rect in fake_stone_rect_list:
                    ...
                    src_negative_sample_error = error_img_with_stone[y0: y1, x0: x1]
...
if __name__ == '__main__':
    ...
    root_dir = 'D:/huifu/WeiChaiDataset_arranged'
    stone_dir = 'C:/Users/zhangBY/Desktop/produce_stone_data/stoneImg'
    ...
    for side in ['left', 'right']:
        ...
        aim_positive_dir = os.path.join(os.path.join(root_dir, side), 'positive_training_data')
        if not os.path.exists(aim_negative_dir):
            os.mkdir(aim_negative_dir)
        if not os.path.exists(aim_positive_dir):
            os.mkdir(aim_positive_dir)
    ...
    for side in ['left', 'right']:
        positive_label_dir = os.path.join(os.path.join(os.path.join(root_dir, side), 'positive_training_data'), 'labels')
        negative_label_dir = os.path.join(os.path.join(os.path.join(root_dir, side), 'negative_training_data'), 'labels')
        positive_label_list = os.listdir(positive_label_dir)
        negative_label_list = os.listdir(negative_label_dir)

        final_positive_label_path = os.path.join(positive_label_dir, 'labels.txt')
        final_negative_label_path = os.path.join(negative_label_dir, 'labels.txt')
        for positive_label_name in positive_label_list:
            positive_label_path = os.path.join(positive_label_dir, positive_label_name)
            print(positive_label_path)
            str_positive = ''
            with open(positive_label_path, "r", encoding='utf-8') as positive_label:
                str_positive_line = positive_label.readline()
                while str_positive_line != '':
                    str_positive += str_positive_line
                    str_positive_line = positive_label.readline()
                positive_label.close()
            with open(final_positive_label_path, "a", encoding='utf-8') as final_positive_label:
                final_positive_label.write(str_positive)
                final_positive_label.close()
